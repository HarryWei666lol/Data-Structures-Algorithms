...
public class MinHeap<T extends Comparable<T>> implements PriorityQueue<T> {
	...
	private Decreaser<T>[] array;// T is a type that can contain e.g. int, double
	private int size;
	...
	public void moveItem(int from, int to) {
		...
		array[from] = array[to];
		array[to] = temp;
		//moves the Decreaser from index "from" to index "to"
		array[from].loc = from;
		//sets array[to].loc = to, the to here is the new value
		array[to].loc = to;
		ticker.tick(5);
		return;
	}
	...
	public Decreaser<T> insert(T thing) {
		...
		//    that.
		//
		Decreaser<T> ans = new Decreaser<T>(thing, this, ++size); // this refers to the minheap itself, creates handle here, Decreaser becomes handle
		//
	...
	public T extractMin() {
		...
		if(size==1) {
			...
			moveItem(1, size);; // the last value now moves to the root
			array[size--] = null; // last value becomes null
			ticker.tick(3);
			heapify(1);
		}
	...
	private void heapify(int where) {
		...
		//  FIXME
		//
		int firstChild = where *2; // location of 1st child
		int secondChild = where*2+1; // location of 2nd child
		if ( where > size ) { // the loc of given where exceeds loc of size => loc of 1st and 2nd child will surely exceed loc of size
			ticker.tick();
			return;
		}
		else if ( firstChild > size ) { // the loc of 1st child exceeds loc of size => loc of where may or may not have exceeded loc of size
			ticker.tick();
			return;
		}
		else if( array[secondChild] == null) { // this parent doesnt have second child
			if( array[firstChild].getValue().compareTo(array[where].getValue()) < 0){
				moveItem(where, firstChild);
				heapify(firstChild);
			}
		}
		else {
			int minChild = where;
			if(array[minChild].getValue().compareTo(array[firstChild].getValue()) >= 0) {
				minChild = firstChild;
				ticker.tick();
			}
			if(array[minChild].getValue().compareTo(array[secondChild].getValue()) >= 0) {
